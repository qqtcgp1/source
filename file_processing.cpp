...
/*
std::ostream& operator << (std::ostream& stream, const struct DE_input input) {
    
    stream << input.i_strategy << '\n' << input.b_bs_flag << '\n' << input.b_bound_constr << '\n' << input.i_genmax << '\n' << input.i_refresh << '\n' << input.i_D << '\n' << input.i_NP << '\n' << input.f_weight << '\n' << input.f_cross << '\n' << input.termination_instance.valueToReach << '\n' << input.termination_instance.worstBestDiff << '\n' << input.termination_instance.coeffOfVariation << '\n' << input.i_seed << '\n';
    
    for (int i = 0; i < input.i_D; ++i)
        stream << input.fa_minbound[i] << '\n';
    
    for (int i = 0; i < input.i_D; ++i)
        stream << input.fa_maxbound[i] << '\n';
    
    
    return stream;
}

std::istream& operator >> (std::istream& stream, struct DE_input& input) {
    char* line = (char*)malloc(999);
    
    line = line_wo_comment(stream); input.i_strategy = atoi(line);
    line = line_wo_comment(stream); input.b_bs_flag = (bool)atoi(line);
    line = line_wo_comment(stream); input.b_bound_constr = (bool)atoi(line);
    line = line_wo_comment(stream); input.i_genmax = atoi(line);
    line = line_wo_comment(stream); input.i_refresh = atoi(line);
    line = line_wo_comment(stream); input.i_D = atoi(line);
    line = line_wo_comment(stream); input.i_NP = atoi(line);
    line = line_wo_comment(stream); input.f_weight = atof(line);
    line = line_wo_comment(stream); input.f_cross = atof(line);
    
    floatT valueToReach, worstBestDiff, coeffOfVariation;
    ...
    input.termination_instance = termination_criteria(valueToReach, worstBestDiff, coeffOfVariation);
    
    line = line_wo_comment(stream);
    if (memcmp(line, "time", 4) == 0) {
        time_t t_temp; time(&t_temp);
        input.i_seed = t_temp;   ///use system time in second for seed
    }
    else input.i_seed = atol(line);
    
    for (int i = 0; i < input.i_D; ++i) {
        line = line_wo_comment(stream); input.fa_minbound[i] = atof(line);
    }
    
    for (int i = 0; i < input.i_D; ++i) {
        line = line_wo_comment(stream); input.fa_maxbound[i] = atof(line);
    }
    ...
    if (input.i_NP > MAXPOP)
    {
        std::cout << "Error! too many points\n";
        exit(EXIT_FAILURE);
    }
...
std::istream& operator>(std::istream& stream, struct DE_input& input) {
    READ_BINARY(stream, input.i_strategy);
    READ_BINARY(stream, input.b_bs_flag);
    READ_BINARY(stream, input.b_bound_constr);
    READ_BINARY(stream, input.i_genmax);
    READ_BINARY(stream, input.i_refresh);
    READ_BINARY(stream, input.i_D);
    READ_BINARY(stream, input.i_NP);
    READ_BINARY(stream, input.f_weight);
    READ_BINARY(stream, input.f_cross);
    READ_BINARY(stream, input.termination_instance.valueToReach);
    READ_BINARY(stream, input.termination_instance.worstBestDiff);
    READ_BINARY(stream, input.termination_instance.coeffOfVariation);
    READ_BINARY(stream, input.i_seed);
    
    stream.read( reinterpret_cast<char*>(input.fa_minbound), sizeof(floatT) * input.i_D);
    stream.read( reinterpret_cast<char*>(input.fa_maxbound), sizeof(floatT) * input.i_D);
    
    return stream;
}

std::ostream& operator<(std::ostream& stream, const struct DE_input& input) {
    WRITE_BINARY(stream, input.i_strategy);
    WRITE_BINARY(stream, input.b_bs_flag);
    WRITE_BINARY(stream, input.b_bound_constr);
    WRITE_BINARY(stream, input.i_genmax);
    WRITE_BINARY(stream, input.i_refresh);
    WRITE_BINARY(stream, input.i_D);
    WRITE_BINARY(stream, input.i_NP);
    WRITE_BINARY(stream, input.f_weight);
    WRITE_BINARY(stream, input.f_cross);
    WRITE_BINARY(stream, input.termination_instance.valueToReach);
    WRITE_BINARY(stream, input.termination_instance.worstBestDiff);
    WRITE_BINARY(stream, input.termination_instance.coeffOfVariation);
    WRITE_BINARY(stream, input.i_seed);
    
    stream.write( reinterpret_cast<const char*>(input.fa_minbound), sizeof(floatT) * input.i_D);
    stream.write( reinterpret_cast<const char*>(input.fa_maxbound), sizeof(floatT) * input.i_D);
    return stream;
}
 */
