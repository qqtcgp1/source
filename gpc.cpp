...
double gpc_polygon_area( const gpc_polygon* const polygon) {
    double area = 0;
    double partial_area = 0;
    for (int i = 0; i < polygon -> num_contours; ++i) {
        partial_area = 0;
        for (int j = 0; j < polygon -> contour[i].num_vertices - 1; ++j) {
            partial_area += (polygon -> contour[i].vertex[j].x * polygon -> contour[i].vertex[j+1].y);
            partial_area -= (polygon -> contour[i].vertex[j].y * polygon -> contour[i].vertex[j+1].x);
        }
        partial_area += (polygon -> contour[i].vertex[polygon -> contour[i].num_vertices - 1].x * polygon -> contour[i].vertex[0].y);
        partial_area -= (polygon -> contour[i].vertex[polygon -> contour[i].num_vertices - 1].y * polygon -> contour[i].vertex[0].x);
        area += fabs(partial_area / 2.0);
    }
    return area;
}

void gpc_translate(gpc_polygon* const polygon, double x, double y) {
    for (int i = 0; i < polygon->num_contours; ++i) {
        for (int j = 0; j < polygon->contour[i].num_vertices; ++j) {
            polygon->contour[i].vertex[j].x += x;
            polygon->contour[i].vertex[j].y += y;
        }
    }
}
